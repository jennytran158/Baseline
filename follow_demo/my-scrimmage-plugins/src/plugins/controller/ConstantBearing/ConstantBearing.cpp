...
#include <my-scrimmage-plugins/plugins/controller/ConstantBearing/ConstantBearing.h>
...
REGISTER_PLUGIN(scrimmage::Controller,
                scrimmage::controller::ConstantBearing,
                ConstantBearing_plugin)

namespace scrimmage {
namespace controller {

void ConstantBearing::init(std::map<std::string, std::string> &params) {
  double p_gain = sc::get("vel_p_gain", params, 1.0);
  double i_gain = sc::get("vel_i_gain", params, 0);
  double d_gain = sc::get("vel_d_gain", params, 0);
  double i_lim = sc::get("vel_i_lim", params, 1.0);
  vel_pid_.set_parameters(p_gain, i_gain, d_gain);
  vel_pid_.set_integral_band(i_lim);
  max_vel_ = std::stod(params["max_vel"]);
}

bool ConstantBearing::step(double t, double dt) {
...
;
    ...
    vel_pid_.set_setpoint((dist < 10) ? 0 : dist);
